...
//Copies string to a new memory allocation
...
char **tokenize(char* str){
  ...
  char *word_end;
  char *word_begin;
  char **tokens = (char**)malloc(sizeof(char*)*total_words); // Tokens are stored
  char *pointer = word_start(str); // Temp pointer
  ...
  while(i < total_words){
    ...
    *tokens = copy_str(word_begin, word_size); // Copying over word to *tokens
    tokens++;
